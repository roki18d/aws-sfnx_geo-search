...
例えば「東京都渋谷区宇田川町」のような住所文字列を入力として、その住所の「周辺施設情報」「地理高度情報」「天気情報」を取得し、これらの結果をレポートとして DB に書き込みます。

## 入力

アプリケーション全体のオーケストレーションは Step Functions (SFN) で実装しており、SFN ステートマシンへの実行入力がアプリケーションへの入力となります。入力フォーマットは以下のように定義しています。クエリ (query) に指定する文字列は、日本の都道府県から大字以降までの住所文字列を想定しています。
...
SFN ステートマシン実行のうち、成功したもののトレースを見てみます。基本的には同期的に処理が進んでおり、地理・地域情報取得の部分は、ステートマシン定義通り並列で実行されています。「地理高度情報」取得のみ、5秒間の一時停止を設定しましたが、やはり性能ボトルネックになっていることがトレースからも分かります。
...
Yahoo! API を使用して AWS アプリケーションを実装し、X-Ray を使って性能・可用性を可視化してみました。X-Ray を利用することでサービス全体としての非機能ボトルネックを特定しやすくなり、改善のためのアクションを検討しやすくなります。

今回は Step Functions, Lambda を対象に X-Ray tracing を有効化してみましたが、他にも様々なサービスが対応しています。また、同期実行パイプラインだけでなく、SQS をまたいだ非同期実行パイプラインにも利用できるようです (参考.2) 個人的には、ETLパイプラインを Step Functions で実装することが多いので、ETL 系サービスにも対応してくれると嬉しいなと思っています。
...
1. [AWS X-Ray | AWS](https://aws.amazon.com/xray/)
2. [AWS X-Ray Now Supports Amazon SQS](https://aws.amazon.com/about-aws/whats-new/2019/08/aws-x-ray-now-supports-amazon-sqs/)
3. [YOLP(地図) | Yahoo! デベロッパーネットワーク](https://developer.yahoo.co.jp/webapi/map/)
